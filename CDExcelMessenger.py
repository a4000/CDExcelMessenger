...
def getColNamesForUpdatingCD(cdResultsFilePath, cursor, peakTable, excelFilePath, updateColNameList):
    try:
        report = []
                    ...
                    if cursor.fetchall()[0][0] != 0:
                        ...
                        if cursor.fetchall()[0][0] != 6 or colDisplayName == "Tags":
                            ...
                            if cursor.fetchall()[0][0] == 1:
                                ...
                                # If the column is not the Name column
                                if colDisplayName != "Name":
                                ...
                                # If the column is the Name column    
                                else:
        ...
        # If the user just wants to get all editable columns
            ...
            for colNameTuple in tempColNameTupleList:
                ...
                if colNameTuple[1] in peakTable.columns:
                    ...
                    # Make sure user doesn't edit the Name column
                    if colNameTuple[1] != "Name":
...
def updateCDResultsFile(cdResultsFilePath, excelFilePath, peakSheetName, updateColNameList = None, tagList = None, verbose = True):
    ...
    try:
        ...
        colNameTupleList, newReport = getColNamesForUpdatingCD(cdResultsFilePath, cursor, peakTable, excelFilePath, updateColNameList)
        for i in newReport:
            report.append(i)
...
def updateExcelFile(cdResultsFilePath, excelFilePath, peakSheetName, updateColNameList = None, removeCheckedRows = False, verbose = True):
    ...
    try:
        ...
        if "Label" in peakTable.columns:
            firstCols.append("Label")
...
def CleanupPeakTable(Peak,Dictionary,options):
    ...
    # Get all rows of 'Peak'. Also get the columns 'Idx', 'Name', 'Label', and any columns that are keys in 'NewDict'
    cols = ['Idx', 'Name', 'Label']
    for key in NewDict:
        if key not in cols:
            cols.append(key)
    ...
    if options["CIMCBlib"]:
        ...
        for i in range(len(Peak.index)):
            currLabel = Peak.at[i,"Label"]
            
            if type(currLabel) != float:
                ...
                if currLabel.startswith("ECU"):
                    ...
                    Peak.at[i,"Label"] = temp[1]    
        
        if 'CIMCBlib' in Peak.columns:
            # Make sure the first four columns of Peak are 'Idx', 'Name', 'CIMCBlib', and 'Label'
            Peak = Peak[['Idx', 'Name', 'CIMCBlib', 'Label'] + [c for c in Peak if c not in ['Idx', 'Name', 'CIMCBlib', 'Label']]] 
        else:
            # Make sure the first three columns of Peak are 'Idx', 'Name', and 'Label'
            Peak = Peak[['Idx', 'Name', 'Label'] + [c for c in Peak if c not in ['Idx', 'Name', 'Label']]] 
...
def validatingDataPeakTables(DataTable, PeakTable, options):
    ...
    # if Name or Label not in peak columns
    if "Name" not in peakHeader or "Label" not in peakHeader:
        raise Exception("TidyData:PeakTableError", "PeakTable must contain columns ''Name'' & ''Label''")

    # Make sure all values in the 'Name' column are unique
    ...
    if DataTable.dtypes["SampleID"] == "int64" or DataTable.dtypes["SampleID"] == "float64":
...
def validateTidyDataInput(excelFilePath, colsToKeepDict, optionsDict, verbose):
    ...
    try:
        ...
        if type(optionsDict["ColumnWarning"]) != bool:
            raise TypeError("TypeError", "Make sure you set 'ColumnWarning' to a boolean value") 
...
def tidyData(excelFilePath, colsToKeepDict, optionsDict, verbose = True):
    try:
        ...
        # Create lists containing peak Idx and the new Name data
        peakIdxList = list(range(1, len(peakTable.index) + 1))
        nameList = [prefix + str(x) for x in peakIdxList]
        
        # Create new columns called 'Idx' and 'Name'  
        peakTable["Idx"] = peakIdxList
        peakTable["Name"] = nameList
        ...
        for col in peakTable.columns:
            if col.startswith("Area: "):
                areaDict[index-1] = peakTable.loc[:, col]
                index = index + 1
