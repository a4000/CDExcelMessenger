            ...
            if peakTable.dtypes["Tags"] == "object":
                ...
                for row in peakTable.index:
                    ...
                    if pd.notnull(peakTable.at[row,"Tags"]):
                        # Get the string in the cell, split the string, then add each tag to a set 
                        ...
                        if cursor.fetchall()[0][0] != cdDataTypeDict["Binary"] or colDisplayName == "Tags":
                            ...
                            if cursor.fetchall()[0][0] == 1:
                                ...
                                if colNameTuple[0] != "originalName":
                                    ...
                                    # If the column is not the MSI column or Tags column
                                    if colNameTuple[0] != "MSI" and colNameTuple[0] != "Tags":
        ...
        # If the user just wants to get all editable columns
            ...
            for colNameTuple in tempColNameTupleList:
                ...
                if colNameTuple[1] in peakTable.columns:
                    ...
                    # Make sure user doesn't edit the originalName, MSI or Tags column
                    if colNameTuple[0] != "originalName" and colNameTuple[0] != "MSI" and colNameTuple[0] != "Tags":
            ...
            for colDisplayName in excelColList:
                ...
                if cursor.fetchall()[0][0] != 0:
                    ...
                    if cursor.fetchall()[0][0] != cdDataTypeDict["Binary"] or colDisplayName == "Tags":
                        ...
                        # Get the column DBName and Display name, put those names in a tuple, then add that tuple to the list of tuples if this isn't the MSI column
                        cursor.execute("SELECT DBColumnName, Property_DisplayName FROM DataTypesColumns WHERE DataTypeID = (?) AND Property_DisplayName = (?);", (compoundTblID, colDisplayName, ))
                        colNameTuple = cursor.fetchall()[0]
...
INPUT:
'Peak' = The peak table.
'Dictionary' = Dictionary containing peak columns to keep and the columns new name.
'options' = Dictionary containing options and the users chosen values for those options.

RETURN:
'PeakNew' = Peak table after cleaning.
...
OUTPUT:
'Data' = The Data table after Meta columns have been added.
'''
...
'options' = Dictionary containing options and the values the user chose for those options.
